...
async function cliente() {
    ...
    await fetch(api + `/clientes/${idcli}`)
...
async function contrato() {
    ...
    var fechafactini = document.getElementById('fechafactini');
    var fechafactfin = document.getElementById('fechafactfin');
    var costorenta = document.getElementById('costorenta');
    ...
    .then(response => response.json())
    .then((data) => infcontrato = data.contrato[0]);
        idcli = infcontrato.idcliente;
        fechaini.value = infcontrato.fechaini;
        fechafin.value = infcontrato.fechafin;
        fechafactini.value = infcontrato.fechafactini;
        fechafactfin.value = infcontrato.fechafactfin;
        plazo.value = infcontrato.idplazo;
        idbienpass = infcontrato.idbien;
        idbien = infcontrato.idbien;
        deposito.value = infcontrato.deposito;
        costorenta.value = infcontrato.monto;
        agrupar.value = infcontrato.agrupado ? 1 : 0;
        if(infcontrato.activo == 1){
            document.getElementById("divactivo").style.display = "block";
            document.getElementById("divinactivo").style.display = "none";
        }else{
            document.getElementById("divactivo").style.display = "none";
            document.getElementById("divinactivo").style.display = "block";
        }


        await fetch(api + `/bienes/${idbienpass}`)
        .then(response => response.json())
...
document.getElementById("indef").addEventListener("click",()=>{
    add = new Date(fechafactini.value).toISOString().slice(0, 10)
    actplazo = "indef";
    return fechafactfin.value = add;
})
document.getElementById("year").addEventListener("click",()=>{
    var hoy = new Date(fechafactini.value)
...
let actplazo;
document.getElementById("fechafactini").addEventListener("change", ()=>{
    switch(actplazo){
        case 'mes': 
...
document.getElementById("renovar").addEventListener("click", (event) => {
    event.preventDefault();
    registrarcontrato()
})
...
async function borrar(){
    var mensaje = confirm("¿Seguro que desea eliminar este contrato?"); 
    if (mensaje) {
        var data = {
            id: info,
            tipo: 'Contrato',
        ...
        xhr.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                window.location.assign(`../../vistas/consultas/clientcontrato.html?inf=${idcli}`);
...
function registrarcontrato() {
    ...
    const fechafactini = document.getElementById('fechafactini').value;
    const fechafactfin = document.getElementById('fechafactfin').value;
    ...
    if( fechafactfin < fechafactini){
        alert("La fecha de final de facturación no puede ser menor a la de inicio de facturación.")
        return false;
    }
    if( fechafactfin=="" ||  fechafactini==""){
        alert("Las fechas de facturación no pueden esta vacias.")
        return false;
    }
    ...
    var data = {
        ...
        idbienpass,
        fechaini: fechaini,
        fechafactini: fechafactini,
        fechafactfin: fechafactfin,
        fechafin: fechafin,
        monto: monto,
        idplazo: idplazo,
        agrupado: agrupar,
    ...
    xhr.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if(xhr.responseText == "concontrato"){
                return alert('El bien seleccionado esta registrado en otro contrato!')
            }else{
                window.location.assign(`../../vistas/consultas/clientcontrato.html?inf=${idcli}`);
    ...
    xhr.open("POST", api + "/contratos/update");
    xhr.setRequestHeader("Accept", "application/json");
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.send(JSON.stringify(data));
...
async function pintarlista() {
    ...
    disponibles.forEach(bien => {
        ...
        <td><a class="btn btn-dark" id="seleccionar" onclick="bien(${bien.idbien})">Seleccionar</a></td>
    </tr>`
    });
    html += `</table>`
